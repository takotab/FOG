...
function res=x_countTxFx(gtframe,lframe,offDelay,onDelay,sampleweight)
    ...
    % EDIT by Tako: Returns now a confusion matrix and added defaults. Also
    % include the possibility of adding sampleweights
    
    if ~exist('onDelay','var') || isempty(onDelay)
        offDelay = 1; %in samples
        onDelay = offDelay;
    end
    if ~exist('sampleweight','var')
       sampleweight = ones(size(gtframe)); 
    end
...
%        
    ...
    % include sampleweight
    res_vec = sampleweight .* res_vec;
